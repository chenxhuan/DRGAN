...
def split_train_test3(inFile, cross_fold = 5):
    ...
    for fold_index in xrange(cross_fold):
        ...
        for index,kv in enumerate(askers):
            ...
            if index < fold_index * fold_size or index > (fold_index + 1)*fold_size:   # train set
                for value in values:
...
def generate_uniform_pair(trainset,fold_index):
    ...
    with codecs.open(path+trainset+str(fold_index),'r') as train:
