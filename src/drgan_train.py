...
from ask120_dataPrepare import generate_uniform_pair, generate_test_samples
...
tf.flags.DEFINE_string("loss", "svm", "")
...
def gan_samples(eb_samples,pro_samples,sess,gen,negative_size=2):
    ...
    for _index, row in enumerate(eb_samples):
        ...
        neg_indexes = np.random.choice(range(batch_size), size= negative_size,p=prob,replace=False) 
                    ...
                    for g_epoch in range(FLAGS.g_epochs_num):
                        ...
                        for _index, row in enumerate(eb_samples):
                            ...
                            feed_dict  = {
                                ...
                                gen.reward    :reward}
                            _, step,current_loss,gan_score,pos_score,neg_score = sess.run(
                                    [gen.gan_updates, gen.global_step, gen.gan_loss, gen.gans,gen.positive,gen.negative],
                                    feed_dict)
                        line=("%s: GEN step %d %d, loss %f  gan score %f ,pos score %f, neg score %f, total step: %d "%(timestamp(), step,i, current_loss,gan_score,pos_score,neg_score,FLAGS.num_epochs*FLAGS.g_epochs_num*len(eb_samples)))
                        print line
                        log.write(line+"\n")
